...
class FuckOpensea:
    ...
    def setup_method(self, env_dist):
        ...
        # 初始化等待器
        self.wait = WebDriverWait(self.driver, int(self.env_dist.get("WAIT_TIME_1")), float(self.env_dist.get("CHECK_INTERVAL")))
        time.sleep(2)
        print("初始化浏览器✅")
    ...
    def verify_metamask(self, menmoicWord):
        self.driver.get('moz-extension://38d67efb-5720-4c2e-8025-c5208d4202a0/home.html#initialize/create-password/import-with-seed-phrase')
        # 助记词输入
        menmoicWord = menmoicWord.split(" ")
        for x in range(0, len(menmoicWord)):
            self.wait.until(EC.visibility_of_element_located((By.XPATH, '''//*[@id="import-srp__srp-word-{0}"]'''.format(x)))).send_keys(menmoicWord[x])
        # 密码输入
        self.driver.find_element(By.XPATH, '//*[@id="password"]').send_keys(self.env_dist.get('METAMASK_PASSWORD'))
        self.driver.find_element(By.XPATH, '//*[@id="confirm-password"]').send_keys(self.env_dist.get('METAMASK_PASSWORD'))
        # 勾选同意
        self.driver.find_element(By.XPATH, '//*[@id="create-new-vault__terms-checkbox"]').click()
        # 点击确定
        self.driver.find_element(By.XPATH, '//*[@id="app-content"]/div/div[2]/div/div/div[2]/form/button').click()
        # 确认完成
        self.wait.until(EC.visibility_of_element_located((By.XPATH, '//*[@id="app-content"]/div/div[2]/div/div/button'))).click()
        print("激活钱包✅")
